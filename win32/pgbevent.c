 ...
 *		The DLL defines event source for pg_ddm tools
 *-------------------------------------------------------------------------
 */
...
#define APP_KEY "SYSTEM\\CurrentControlSet\\Services\\EventLog\\Application\\pg_ddm"
...
STDAPI DllRegisterServer(void)
{
	...
	if (!GetModuleFileName((HMODULE)g_module, buffer, sizeof(buffer))) {
		MessageBox(NULL, "Could not retrieve DLL filename", "pg_ddm error", MB_OK | MB_ICONSTOP);
	...
	if (RegCreateKey(HKEY_LOCAL_MACHINE, APP_KEY, &key)) {
		MessageBox(NULL, "Could not create the registry key.", "pg_ddm error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}

	/* Add the name to the EventMessageFile subkey. */
	if (RegSetValueEx(key, "EventMessageFile", 0, REG_EXPAND_SZ, (LPBYTE)buffer, strlen(buffer) + 1)) {
		MessageBox(NULL, "Could not set the event message file.", "pg_ddm error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
	...
	if (RegSetValueEx(key, "TypesSupported", 0, REG_DWORD, (LPBYTE)&data, sizeof(DWORD))) {
		MessageBox(NULL, "Could not set the supported types.", "pg_ddm error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
...
STDAPI DllUnregisterServer(void)
{
	if (RegDeleteKey(HKEY_LOCAL_MACHINE, APP_KEY)) {
		MessageBox(NULL, "Could not delete the registry key.", "pg_ddm error", MB_OK | MB_ICONSTOP);
		return SELFREG_E_TYPELIB;
	}
	return S_OK;
}
